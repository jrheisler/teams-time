...
function buildTimezoneMetadataMap(metadata) {
  ...
  for (const entry of entries) {
    ...
    const zoneLabel = entry.displayLabel || entry.label || zoneId;
...
async function initialize() {
  ...
  const timezoneMetadataByZone = buildTimezoneMetadataMap(timezoneMetadata);

  populateTimezoneDatalist(zoneList, FALLBACK_TIMEZONE_ALIASES, timezoneMetadataByZone);
...
function normalizeTimezoneInputValue() {
...
timezoneInput.addEventListener('change', normalizeTimezoneInputValue);
timezoneInput.addEventListener('blur', normalizeTimezoneInputValue);
